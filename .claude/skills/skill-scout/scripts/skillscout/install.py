...
REGISTRY = Path.home() / ".claude" / "skill-scout" / "registry.json"
...
def _safe_name(name: str) -> str:
    return re.sub(r"[^a-zA-Z0-9._-]", "-", name).strip("-") or "skill"
...
def install_skill(repo_url: str, name: str, sha: str = "", band: int | None = None,
                  score: int | None = None, dest_dir: Path | None = None) -> dict:
    ...
    tmp = tempfile.mkdtemp(prefix="skillscout-install-")
    try:
