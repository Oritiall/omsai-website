...
description: >
  ...
  accent, the three-step gray text ladder, Rubik (display/numerals/chrome) + Rubik (body),
...
colors:
  ...
  accent-light: "rgba(0, 191, 179, 0.10)"
  accent-medium: "rgba(0, 191, 179, 0.18)"
  border: "rgba(0, 191, 179, 0.30)"
  card-bg: "rgba(0, 191, 179, 0.07)"
  positive: "#34D399"
  negative: "#F87171"
...
typography:
  # — reading ramp (Rubik body + Rubik chrome) —
  body:    { fontFamily: "Rubik", cqw: 0.85, weight: 400, lineHeight: 1.6, color: "text-muted" }
  h4-eyebrow:{ fontFamily: "Rubik", cqw: 0.8, weight: 600, tracking: "0.08em", upper: true, color: "primary" }
  tag:     { fontFamily: "Rubik", px: 12, weight: 500, color: "primary" }
  counter: { fontFamily: "Rubik", px: 13, weight: 500, tracking: "0.05em", color: "text-muted" }
  # — display / numerical ramp (Rubik, near-black headings / cobalt numerals) —
  h3:      { fontFamily: "Rubik", cqw: 1.25, weight: 500, lineHeight: 1.3, tracking: "-0.02em", color: "text" }
  stat-num:{ fontFamily: "Rubik", cqw: 1.9, weight: 700, lineHeight: 1.0, color: "primary" }
  blockquote:{ fontFamily: "Rubik", cqw: 2.4, weight: 500, lineHeight: 1.35, color: "text" }
  h2:      { fontFamily: "Rubik", cqw: 2.6, weight: 600, lineHeight: 1.1, tracking: "-0.02em", color: "text" }
  metric-value:{ fontFamily: "Rubik", cqw: 3.0, weight: 700, lineHeight: 1.0, color: "primary" }
  h1:      { fontFamily: "Rubik", cqw: 4.2, weight: 700, lineHeight: 1.08, tracking: "-0.02em", color: "text" }
  quote-mark:{ fontFamily: "Rubik", cqw: 8.0, weight: 700, lineHeight: 0.5, color: "primary", opacity: 0.15 }
...
- **Fonts** — already set to **Rubik** (display) / **Arial Hebrew** (body); ignore any preset font name lingering in prose.
- **Colors** — use the frontmatter hex; preset color NAMES in prose (e.g. "cobalt", "cream") mean the remapped brand values.
- **Canvas is dark, not light.** The site's brand palette is a dark navy system (per BRIEF.md), so this remix was hand-adjusted to invert the preset's canvas/text roles: `{colors.bg}` is dark navy `#0B2447` (not cream), `{colors.text}` is white/near-white (not near-black), teal `{colors.primary}` `#00BFB3` remains the single accent. Every mention of "cream", "warm cream canvas", or "near-black headline" in the prose below means the dark-navy canvas + white headline instead — read literally as dark-mode, not light-mode.
...
The voice is two faces in fixed roles: **Rubik** (display, every numeral, all chrome —
eyebrows uppercase 0.08em) and **Rubik** (body, muted gray, line 1.6). Headlines are near-black;
...
- **Rubik** (display/numerals/chrome) + **Rubik** (body) — near-black headlines, cobalt numerals.
...
Two ramps. The **reading ramp** (Rubik body 0.85cqw muted; Rubik eyebrow uppercase 0.08em
cobalt) carries copy + chrome; the **display/numerical ramp** (Rubik `h3` 1.25cqw → `h1`
...
- **Headlines near-black, −0.02em**; **eyebrows cobalt, uppercase, 0.08em**; **numerals cobalt 600–700**; **body Rubik 400 muted, line 1.6**. No italic, no uppercase body, no cobalt headline.
...
**Composes** accent-line, meta, h1, body sub. **Focal** a 2-line Rubik `h1` near-black, left,
...
**Focal** a row of tinted cards — cobalt `metric-value` + Rubik label + muted desc + optional
...
blockquote, cite. **Focal** a Rubik `blockquote` near-black under a 15%-opacity cobalt
...
Rubik body column beside a cobalt-8% highlight block (4px cobalt left rule) carrying an inline pull
...
a Rubik `h1` near-black, centered, with the one solid cobalt `cta-button` pill below. **Accent**
...
- Use **tinted cards** (4% fill, 20% border, 10–14px radius, no shadow); body Rubik 400 muted, line 1.6.
...
- **Type** — Rubik headings −0.02em near-black, cobalt eyebrows 0.08em + numerals; Rubik body muted line 1.6; ≥1.4cqw floor.
...
- **Rubik + Rubik via Google Fonts.** CJK pairing (Noto Sans SC 700 display / Noto Serif SC 400 body) carries over; the eyebrow's uppercase+tracking signal weakens in CJK — pair it with the accent-line.
